...
const char* action_channel1 = "home/bedroom/temperature";
        ...
        public:
                ...
                virtual void on_message(const struct mosquitto_message* mosqmessage) {
                       ...
                       // Create message to send via RF24
                        ...
                        uint8_t*  mensaje=mosqmessage->payload;
                        int val=atoi((const char *)mensaje);
                        printf("%i",val);
                         actionmessage=(message_action){(float)val };

                        //7float fmensaje=atof(mensaje);

                //      float num = (float) mosqmessage->payload;
                        //unsigned int a = new unsigned int(10);
                        //float b = (float)( mosqmessage->payload);
                        ...
                        if (header.type == '1') {
                                ...
                                switch (header.from_node) {
                                        case sensor_node1:
                                                ...
                                                system(buffer);
                                                valor=sensormessage.temperature;
                                                printf("%f", valor);
